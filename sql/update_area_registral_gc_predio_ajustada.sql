...
--
-- Destino (v3): Schema configurable via cun25436_migrado_nuevo_modelo
...
--
-- IMPORTANTE:
--   1. Reemplazar cun25436_migrado_nuevo_modelo con el schema destino (ej: cun25436)
--   2. Ajustar la cadena de conexion dblink si la BD CCA esta en otro servidor/puerto
--   3. Ajustar cun25436 si el schema CCA tiene otro nombre
...
SELECT 'Cruzan por numero_predial Y nupre' AS descripcion,
       count(*) AS total
FROM cun25436_migrado_nuevo_modelo.gc_predio gp
...
SELECT 'Registros destino con area_registral_m2 NULL' AS descripcion,
       count(*) AS total
FROM cun25436_migrado_nuevo_modelo.gc_predio
...
UPDATE cun25436_migrado_nuevo_modelo.gc_predio gp
