...
codigo_orip, 
case 
	when p.matricula_inmobiliaria ~ '^[0-9]+$'  then p.matricula_inmobiliaria
	else NULL
...
where av.vigencia = (select max(vigencia) from {schema}.extavaluo av2 where av2.gc_predio_avaluo = p.id LIMIT 1);