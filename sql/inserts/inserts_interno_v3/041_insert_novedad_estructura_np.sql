...
SELECT
    ...
    -- tipo_novedad: Mapeo de valores GC_Django a valores GC_modelo_interno version 3
     COALESCE(
        -- Intento 1: Coincidencia exacta
        (SELECT t_id FROM {schema}.gc_estructuranovedadnumeropredial_tipo_novedad
         WHERE ilicode = e.tipo_novedad LIMIT 1),
        -- Intento 2: Coincidencia parcial (el valor GC_Django está contenido en ilicode de GC_modelo_interno_v3)
        (SELECT t_id FROM {schema}.gc_estructuranovedadnumeropredial_tipo_novedad
         WHERE ilicode ILIKE e.tipo_novedad || '.%' LIMIT 1),
        -- Intento 3: Coincidencia inversa (el ilicode de GC_modelo_interno_v3 contiene el valor gc_django)
        (SELECT t_id FROM {schema}.gc_estructuranovedadnumeropredial_tipo_novedad
         WHERE ilicode ILIKE '%' || e.tipo_novedad || '%' LIMIT 1),
...
FROM tmp_estructura_novedad_np e
WHERE EXISTS (
